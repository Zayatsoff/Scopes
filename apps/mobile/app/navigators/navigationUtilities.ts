...
export function useNavigationPersistence(
  ...
  customStateHandler?: (state: any) => NavigationProps["initialState"],
) {
  ...
  const restoreState = async () => {
    try {
      ...
      // If customStateHandler is provided, use it instead of restoring from storage
      if (customStateHandler) {
        const customState = customStateHandler(null)
        setInitialNavigationState(customState)
        return
      }
