...
export class TrafficAlertsApi {
  ...
  async getTrafficAlerts() {
    try {
      ...
      const response = await this.apisauce.get<{ trafficAlerts: { events: TrafficAlertItem[] } }>(
      ...
      if (response.ok) {
        if (__DEV__) {
          console.log(
            "Traffic Alerts API Response successful, items:",
            response.data?.trafficAlerts?.events?.length || 0,
          )

          // Check if we actually got data even though the response was "ok"
          if (
            !response.data ||
            !response.data.trafficAlerts ||
            !response.data.trafficAlerts.events
          ) {
          ...
          if (response.data?.trafficAlerts?.events?.[0]) {
            console.log(
              "First traffic alert item sample:",
              JSON.stringify(response.data.trafficAlerts.events[0]),
            )
          }
        }
      } else {
        console.error(
          "Traffic Alerts API Error Response:",
          response.problem,
          response.status,
